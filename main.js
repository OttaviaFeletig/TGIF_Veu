var dataObj = new Vue({
  ...
  computed: {
    filter: function() {
      if (!this.checkedParty.length && this.checkedState == "all") {
        ...
        return this.politician.filter(
          x =>
            this.checkedParty.includes(x.party) || this.checkedState == x.state
        );
      } else {
        return this.politician.filter(
          x =>
  ...
  methods: {
    ...
    getMembersNumber: function() {
      this.democratsMembersList = this.politician.filter(onePolitician =>
        onePolitician.party.includes("D")
      );

      this.democratsMembersNumber = this.democratsMembersList.length;

      this.republicansMembersList = this.politician.filter(onePolitician =>
        onePolitician.party.includes("R")
      );

      this.republicansMembersNumber = this.republicansMembersList.length;

      this.independentsMembersList = this.politician.filter(onePolitician =>
        onePolitician.party.includes("I")
      );

      this.independentsMembersNumber = this.independentsMembersList.length;

      this.totalMembersNumber =
        this.democratsMembersNumber +
        this.republicansMembersNumber +
        this.independentsMembersNumber;
    },
    getMembersPct: function() {
      this.democratsMembersPct =
        Math.round(
          (this.democratsMembersList
            .map(pct => pct.votes_with_party_pct)
            .reduce((a, b) => a + b, 0) /
            this.democratsMembersList.length) *
            100
        ) / 100 || 0;

      this.republicansMembersPct =
        Math.round(
          (this.republicansMembersList
            .map(pct => pct.votes_with_party_pct)
            .reduce((a, b) => a + b, 0) /
            this.republicansMembersList.length) *
            100
        ) / 100 || 0;

      this.independentsMembersPct =
        Math.round(
          (this.independentsMembersList
            .map(pct => pct.votes_with_party_pct)
            .reduce((a, b) => a + b, 0) /
            this.independentsMembersList.length) *
            100
        ) / 100 || 0;

      this.totalMembersPct =
        Math.round(
          ((this.democratsMembersPct +
            this.republicansMembersPct +
            this.independentsMembersPct) /
    ...
    getLoyaltyList: function() {
      this.votes10PercentMostLoyal = this.politician
        .map(el => el.votes_with_party_pct)
        .sort((a, b) => b - a)[this.index10Pct];

      this.votes10PercentLeastLoyal = this.politician
        .map(el => el.votes_with_party_pct)
        .sort((a, b) => a - b)[this.index10Pct];
    ...
    getAttendanceList: function() {
      this.votes10PercentMostEngaged = this.politician
        .map(el => el.missed_votes_pct)
        .sort((a, b) => a - b)[this.index10Pct];

      this.votes10PercentLeastEngaged = this.politician
        .map(el => el.missed_votes_pct)
        .sort((a, b) => b - a)[this.index10Pct];
